...
## 3. 资产与凭证隔离架构
* 配置文件：`mail-service/profiles/edu/profile.json`（可从 `profile.sample.json` 复制）
* 凭据文件：`mail-service/profiles/edu/.env`（受 `.gitignore` 保护）
* 个性化签名：`mail-service/profiles/edu/signature.html`（受 `.gitignore` 保护）
* 通讯录：`mail-service/profiles/edu/contacts.json`
* 独立附件下载：`mail-service/profiles/edu/downloads/`
* 审计日志：`mail-service/profiles/edu/mail.log`
...
```powershell
# 1. 快捷专属入口
.\mail-service\edu.ps1 test
.\mail-service\edu.ps1 check --limit 5
.\mail-service\edu.ps1 search --query "录取通知"
