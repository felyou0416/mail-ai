...
class MailAIRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 1. API: 系统体检
        if path == "/api/doctor":
            script = os.path.join(PROJECT_ROOT, "core", "system-doctor.ps1")
        ...
        if path == "/api/drafts":
            ...
            script = os.path.join(PROJECT_ROOT, "core", "draft-manager.ps1")
        ...
        if path == "/api/attachments":
            script = os.path.join(PROJECT_ROOT, "..", "taoci", "core", "engine", "attachment-manager.ps1")
        ...
        if path == "/api/scan":
            via = query.get("via", ["edu"])[0]
            script = os.path.join(PROJECT_ROOT, "core", "mailbox-inspector.ps1")
            data = run_ps1_json([script, "scan", "-Json", "--via", via])
            return self.send_json(data)

        # 6. API: 个人画像与配置
        if path == "/api/profile":
            p_file = os.path.join(PROJECT_ROOT, "..", "taoci", "personal", "profile.json")
            if not os.path.exists(p_file):
                p_file = os.path.join(PROJECT_ROOT, "..", "taoci", "core", "schemas", "student_profile.sample.json")
            profile_data = {}
    ...
    def do_POST(self):
        ...
        if path == "/api/check_mentor":
            ...
            script = os.path.join(PROJECT_ROOT, "..", "taoci", "core", "engine", "mentor-checker.ps1")
            args = [script, "-Json"]
            if name:
