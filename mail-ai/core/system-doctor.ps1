...
$script:SKILL_DIR = Join-Path $script:PROJECT_ROOT "taoci"
...
# mail-skill (智能检索与本地向量库)
if (-not $Json) { Write-Host "  - 正在检测 mail-skill 智能本地库引擎 ... " -NoNewline }
$skillCli = Join-Path $script:PROJECT_ROOT "mail-ai\mail-skill\scripts\mail_cli.py"
$skillStatus = "missing"
$skillAccountsCount = 0
if (Test-Path $skillCli) {
    try {
        $testPython = & python -c "
import sys, os
sys.path.insert(0, os.path.join(r'$($script:PROJECT_ROOT.Replace('\', '/'))', 'mail-ai', 'mail-skill', 'scripts'))
try:
    import imap_tools, bs4
    from mail_manager.config_manager import load_config
    cfg = load_config()
    print('OK|' + str(len(cfg.get('ACCOUNTS', {}))))
except Exception as e:
    print('ERR|' + str(e))
" 2>&1
        if ($testPython -match 'OK\|(\d+)') {
            $skillAccountsCount = [int]$matches[1]
            $skillStatus = "ok"
            if (-not $Json) { Write-Host "[正常 ✓] (Python 依赖就绪，已关联 $skillAccountsCount 个账户)" -ForegroundColor Green }
        } else {
            $skillStatus = "degraded"
            if (-not $Json) { Write-Host "[部分受限] (缺少 Python 依赖，建议 pip install -r mail-ai/mail-skill/requirements.txt)" -ForegroundColor Yellow }
        }
    } catch {
        $skillStatus = "degraded"
        if (-not $Json) { Write-Host "[异常] 无法执行 Python 诊断" -ForegroundColor Yellow }
    }
} else {
    if (-not $Json) { Write-Host "[跳过] (未安装 mail-skill 模块)" -ForegroundColor DarkGray }
}
$channelResults["mail-skill"] = @{ status = $skillStatus; accounts = $skillAccountsCount }
