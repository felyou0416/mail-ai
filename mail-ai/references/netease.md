...
* **高安全加密 (DPAPI)**：
  * 已配置 Windows 原生 DPAPI 加密凭据：`mail-ai/profiles/netease/.credential`
  * 仅当前机器与当前登录用户可自动解密，跨机器/跨用户无法盗取。
  * 若需重新配置，运行：`.\mail-ai\core\setup-credential.ps1 -Profile netease`
* **备用凭据**：
  * `mail-ai/profiles/netease/.env`
* **环境隔离**：
...
## 4. 专属资产隔离
* **专属 HTML 签名**：
  * 路径：`mail-ai/profiles/netease/signature.html`
  * 内容：商务正式落款与联络卡片。
* **专属附件下载目录**：
  * 路径：`mail-ai/profiles/netease/downloads/`
  * 商务合同、商业报表、求职简历等附件独立保存。
* **专属审计日志**：
  * 路径：`mail-ai/profiles/netease/mail.log`
...
.\mail-ai\core\mail-engine.ps1 -Account netease test

# 查看收件箱
.\mail-ai\core\mail-engine.ps1 -Account netease check --limit 5

# 搜索包含应聘相关邮件
.\mail-ai\core\mail-engine.ps1 -Account netease search --query "面试"

# 发送正式业务邮件（自动附带 163 商务签名）
.\mail-ai\core\mail-engine.ps1 -Account netease send --to "hr@company.com" --subject "应聘材料 - 姓名" --body "尊敬的HR：您好，附件为最新简历材料，请查收。" --attach "resume.pdf"
```
