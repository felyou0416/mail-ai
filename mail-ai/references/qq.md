...
* **隔离存储**：
  * 配置文件：`mail-ai/profiles/qq/profile.json`
  * 凭据文件：`mail-ai/profiles/qq/.env`
  * DPAPI 加密凭据：`mail-ai/profiles/qq/.credential`

---

## 4. 专属资产隔离
* **专属 HTML 签名**：
  * 路径：`mail-ai/profiles/qq/signature.html`
  * 内容：简洁的个人签名样式，发信时引擎自动注入。
* **专属附件下载目录**：
  * 路径：`mail-ai/profiles/qq/downloads/`
  * QQ 邮箱接收的私人生活附件完全存储于该目录。
* **专属审计日志**：
  * 路径：`mail-ai/profiles/qq/mail.log`
...
```powershell
# 测试连接
.\mail-ai\core\mail-engine.ps1 -Account qq test

# 查看最新邮件
.\mail-ai\core\mail-engine.ps1 -Account qq check --limit 10

# 搜索包含关键字的邮件
.\mail-ai\core\mail-engine.ps1 -Account qq search --query "账单"

# 发送邮件（自动附带 QQ 签名）
.\mail-ai\core\mail-engine.ps1 -Account qq send --to "friend@example.com" --subject "周末安排" --body "详细日程请查收。"
```
