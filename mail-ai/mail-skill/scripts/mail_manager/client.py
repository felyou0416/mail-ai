...
try:
    ...
    _IMAP_TOOLS_AVAILABLE = True
except ImportError:
    _IMAP_TOOLS_AVAILABLE = False
    imap_tools = None  # type: ignore
    A = None  # type: ignore
...
def _html_to_text(html_content: str) -> str:
    """Convert HTML content to plain text with BeautifulSoup or regex fallback."""
    if not html_content:
        return ""
...
class MailClient:
    ...
    def _get_mailbox(self) -> MailBox:
        if not _IMAP_TOOLS_AVAILABLE:
            raise MailClientError(
                "Dependency 'imap-tools' is not installed. Please run: pip install imap-tools beautifulsoup4"
            )
