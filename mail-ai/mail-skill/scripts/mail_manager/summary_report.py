...
def format_summary_report(
    ...
    date_from: date,
    date_to: date,
    sender_summaries: dict[str, list[tuple[dict[str, Any], EmailSummary]]] | None = None,
    overall: OverallSummary | None = None,
    reply_info: dict[str, ReplyInfo] | None = None,
    sender_emails: dict[str, list[dict[str, Any]]] | None = None,
) -> str:
    """Format complete summary report as Markdown."""
    if sender_summaries is None:
        sender_summaries = {}
        if sender_emails:
            for sender, em_list in sender_emails.items():
                sender_summaries[sender] = [
                    (
                        em,
                        EmailSummary(
                            one_liner=(em.get("body_text", "") or "")[:150].replace("\n", " ").strip(),
                            key_points=[(em.get("body_text", "") or "")[:100].replace("\n", " ").strip()] if em.get("body_text") else [],
                            action_items=[],
                            priority="normal",
                        ),
                    )
    ...
    if overall is None:
        total_emails = sum(len(items) for items in sender_summaries.values())
        overall = OverallSummary(
            overview=f"Aggregated summary of {total_emails} emails from {len(sender_summaries)} senders."
        )
